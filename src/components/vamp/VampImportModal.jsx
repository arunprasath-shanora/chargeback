...
export default function VampImportModal({ onClose, onSaved }) {
  const [tab, setTab] = useState("manual"); // manual | csv | api
  ...
  const [apiNetwork, setApiNetwork] = useState("Visa");
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Add TC40, TC15, TC05 (settled transactions) and CE3.0 data per MID per month</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-4">
          ...
          {result && (
            <div className={`flex items-center gap-3 rounded-2xl p-3 mb-4 mt-2 ${result.failed > 0 ? "bg-amber-50 border border-amber-200" : "bg-emerald-50 border border-emerald-200"}`}>
              ...
              <p className="text-sm text-slate-700">{result.success} record{result.success !== 1 ? "s" : ""} saved successfully. {result.failed > 0 ? `${result.failed} failed.` : ""}</p>
          ...
          {tab === "api" && (
            <div className="space-y-4 mt-2">
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800">
                ...
                API import calls your processor's endpoint directly. Provide the base URL, API key, and the reporting period to fetch settled/fraud data.
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-slate-500 mb-1 block">Processor API Endpoint (Base URL)</label>
                  <input value={apiEndpoint} onChange={e => setApiEndpoint(e.target.value)}
                    placeholder="https://api.processor.com/v1/reports"
                    className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-500 mb-1 block">API Key / Bearer Token</label>
                  <input type="password" value={apiKey} onChange={e => setApiKey(e.target.value)}
                    placeholder="sk-..."
                    className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-500 mb-1 block">Reporting Period (YYYY-MM)</label>
                  <input value={apiPeriod} onChange={e => setApiPeriod(e.target.value)}
                    placeholder="2025-01"
                    className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-500 mb-1 block">Card Network</label>
                  <select value={apiNetwork} onChange={e => setApiNetwork(e.target.value)}
              ...
              {apiResult && (
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-mono text-slate-600 whitespace-pre-wrap max-h-40 overflow-auto">{apiResult}</div>
              )}
              <p className="text-xs text-slate-400">
                ℹ️ After entering details, your IT team or Shanora support can configure the exact field mappings for this processor. Contact support for custom API integrations.
              </p>
            </div>
          )}
        </div>
